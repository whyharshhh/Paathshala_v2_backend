...
async function testAdminEndpoints() {
  ...
  if (!ADMIN_TOKEN) {
    console.log("❌ ADMIN_TOKEN not found in environment variables");
    console.log("Please set ADMIN_TOKEN in your .env file");
    console.log("You can get the token by logging in as admin user");
    return;
  }
  ...
  try {
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        ...
        console.log("2. Login as admin and get the JWT token");
        console.log("3. Set ADMIN_TOKEN in your .env file");
      }
    }
  }
}

// Function to help with admin login
async function helpWithAdminLogin() {
  console.log("\n🔧 Admin Login Help:");
  console.log("1. First, create an admin user:");
  console.log("   node scripts/createAdmin.js 'Admin Name' 'Admin Last' 'admin@example.com' 'password123'");
  
  console.log("\n2. Start your server:");
  console.log("   npm start");
  
  console.log("\n3. Login as admin using the login endpoint:");
  console.log("   POST /api/v1/auth/login");
  console.log("   Body: { email: 'admin@example.com', password: 'password123' }");
  
  console.log("\n4. Copy the JWT token from the response and set it in your .env file:");
  console.log("   ADMIN_TOKEN=your_jwt_token_here");
  
  console.log("\n5. Then run this test script again:");
  console.log("   node scripts/testAdmin.js");
}

// Run the test
if (require.main === module) {
  if (!ADMIN_TOKEN) {
    console.log("❌ ADMIN_TOKEN not found");
    helpWithAdminLogin();
  } else {
    testAdminEndpoints();
  }
...
module.exports = { testAdminEndpoints, helpWithAdminLogin }; 